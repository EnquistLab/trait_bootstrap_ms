...
#################################################
  ...
  #Lets do one other simulation with Panama trees dataset

  source("r_functions/sim_sample_size_multidimensional.R")
  
  # Panama
    #since some traits are incomplete, I'll have to toss those
  ...
  #toss observations with NAs
    
    panama_traits <-
      panama_traits %>%
        group_by(ID) %>%
        summarise(non_na=across(value,~length(which(!is.na(.x))))) %>%
        ungroup() %>%
        inner_join(y = panama_traits) %>%
        dplyr::filter(non_na$value == 8) %>%
          select(-non_na)
  ...
  # check that the traits are all represented completely  
    panama_traits %>%
      group_by(trait)%>%
      summarise(n_measures = n())
    
  # toss unneeded columns for clarify
    panama_traits %>%
      select(site,taxon,ID,trait,value) -> panama_traits

  #Run sample size sims
  md_output_panama <- sim_sample_size_multidimensional(tidy_traits = panama_traits,
                                                   community = panama_community,
                                                   n_to_sample = 
                                                     (1:ceiling(x = max(panama_samples$abundance)^.5))^2,
                                                   n_reps_trait = 10,
                                                   ...
                                                   FRic = TRUE, #throws errors at low sampling.
                                                   FDiv = TRUE,
                                                   tempoutRDS = "output_data/temp_multidimensional_panama.RDS",
                                                   continue = TRUE)
  
  
  # saveRDS(object = md_output_panama,
  #         file = "output_data/multidimensional_bootstrap_sample_size_and_method_sims_panama.RDS")
  
  md_output_panama <-
    readRDS("output_data/multidimensional_bootstrap_sample_size_and_method_sims_panama.RDS")
