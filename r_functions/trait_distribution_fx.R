...
trait_distributions_parametric_manual<-function(number_replicates, abundance_data, trait_data,dist_type="normal"){
  ...
  for(t in 2:ncol(trait_data)){
    ...
    if(!dist_type %in%c("normal","lognormal")){"That distribution isn't programed yet"}
      ...
      for( i in 1:nrow( abundance_data)){
        ...
        if(dist_type=="normal"){
          if(nrow(trait_ti)!=0){  
            ...
            if(nrow(trait_ti)>1){
              fit_ti<-NULL
              try(fit_ti<-fitdist(data = trait_ti[,1],distr = "norm"),silent = T)
              if(is.null(fit_ti)){fit_ti<-fitdist(data = trait_ti[,1],distr = "norm",method = "mme")}
              
              rep_n<-c(rep_n,rnorm(n = abund_i,mean = fit_ti$estimate[1],sd = fit_ti$estimate[2]))
