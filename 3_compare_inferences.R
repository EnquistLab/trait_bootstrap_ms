...
#Add parametric bootstrapping comparison to non-parametrics and CWM approaches.
...
colnames(simulation_output)<-c("sample_size","replicate","total_samples","trait","site","moment","boot_mean","cwm_mean","parametric_mean","actual_mean","SES_boot_v_cwm","SES_boot_v_actual","in_95ci","in_100ci","in_95ci_para","in_100_ci_para")
...
ggplot(data = ci_output[which(ci_output$moment=="mean"),], aes(x= log10(sample_size), y=proportion_ci95_correct,color=method))+
  geom_point(position=position_dodge(width = .1))+ylim(c(0,1))+
  facet_wrap(~trait+site)+ggtitle("mean")

ggplot(data = ci_output[which(ci_output$moment=="variance"),], aes(x= log10(sample_size), y=proportion_ci95_correct,color=method))+
  geom_point(position=position_dodge(width = .1))+ylim(c(0,1))+
  facet_wrap(~trait+site)+ggtitle("variance")

ggplot(data = ci_output[which(ci_output$moment=="skewness"),], aes(x= log10(sample_size), y=proportion_ci95_correct,color=method))+
  geom_point(position=position_dodge(width = .1))+ylim(c(0,1))+
  facet_wrap(~trait+site)+ggtitle("skewness")

ggplot(data = ci_output[which(ci_output$moment=="kurtosis"),], aes(x= log10(sample_size), y=proportion_ci95_correct,color=method))+
  geom_point(position=position_dodge(width = .1))+ylim(c(0,1))+
  facet_wrap(~trait+site)+ggtitle("kurtosis")

ggplot(data = ci_output[which(ci_output$moment=="mean"),], aes(x= log10(sample_size), y=proportion_ci100_correct,color=method))+
  geom_point(position=position_dodge(width = .1))+ylim(c(0,1))+
  facet_wrap(~trait+site)+ggtitle("mean")

ggplot(data = ci_output[which(ci_output$moment=="variance"),], aes(x= log10(sample_size), y=proportion_ci100_correct,color=method))+
  geom_point(position=position_dodge(width = .1))+ylim(c(0,1))+
  facet_wrap(~trait+site)+ggtitle("variance")

ggplot(data = ci_output[which(ci_output$moment=="skewness"),], aes(x= log10(sample_size), y=proportion_ci100_correct,color=method))+
  geom_point(position=position_dodge(width = .1))+ylim(c(0,1))+
  facet_wrap(~trait+site)+ggtitle("skewness")

ggplot(data = ci_output[which(ci_output$moment=="kurtosis"),], aes(x= log10(sample_size), y=proportion_ci100_correct,color=method))+
  geom_point(position=position_dodge(width = .1))+ylim(c(0,1))+
  facet_wrap(~trait+site)+ggtitle("kurtosis")
